...
by_views=df.sort_values("별점")
print("\n === 별점 낮은순 ===")
print(by_views[["웹툰제목","별점"]])

# 3. 여러 기준으로 정렬 : 장르별로 묶고, 그 안에서 조회수 내림차순
by_genre_views=df.sort_values(["장르","조회수"], ascending=[True, False])
print("\n === 장르별 -> 조회수 내림차순 ===")
print(by_genre_views[["장르","웹툰제목","조회수"]])

# 4. rank()로 순위 컬럼 만들기 (조회수 기준 내림차순 순위)
df["조회수순위"]=df["조회수"].rank(ascending=False).astype(int)
print("\n=== 순위 컬럼 추가 ===")
print(df[["웹툰제목","조회수","조회수순위"]].sort_values("조회수순위"))

# 5. nlargest / nsmallest: 상위 n개만 빠르게 뽑기
print("\n=== 조회수 TOP 3 (nlargest) ===")
print(df.nlargest(3, "조회수"))